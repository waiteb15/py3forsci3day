...
def get_age_extremes(youngest=True):
    all_raw = get_all_data()
    all_cooked = sorted(all_raw,key=get_age_at_term_end,reverse=youngest)
    return all_cooked[-1]

def get_oldest():
    return get_age_extremes(False)

def get_youngest():
    return get_age_extremes()
